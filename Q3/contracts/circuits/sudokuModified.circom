...
template sudoku() {
    ...
    component puzzle = RangeProof(64);
    component solution = RangeProof(64);

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzzle[i][j] = RangeProof(64);
            puzzle[i][j].in <== puzzle[i][j];
            puzzle[i][j].range[0] <== 0;
            puzzle[i][j].range[1] <== 9;
            assert(puzzle[i][j].out == 1);

            solution[i][j] = RangeProof(64);
            solution[i][j].in <== solution[i][j];
            solution[i][j].range[0] <== 0;
            solution[i][j].range[1] <== 9;
            assert(solution[i][j].out == 1);
